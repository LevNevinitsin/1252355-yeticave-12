...
<form class="form container <?= $errors !== [] ? 'form--invalid' : '' ?>" action="/sign-up.php" method="post" autocomplete="off">
    ...
    <div class="form__item <?= getErrorClassname($errors, 'name') ?>">
        ...
        <input id="name" type="text" name="name" placeholder="Введите имя" value="<?= getPostVal($formData, 'name') ?>">
        <span class="form__error"><?= getErrorMessage($errors, 'name') ?></span>
    </div>
    <div class="form__item <?= getErrorClassname($errors, 'message') ?>">
        <label for="message">Контактные данные <sup>*</sup></label>
        <textarea id="message" name="message" placeholder="Напишите как с вами связаться"><?= getPostVal($formData, 'message') ?></textarea>
        <span class="form__error"><?= getErrorMessage($errors, 'message') ?></span>
    </div>
