...
<form class="form form--add-lot container <?= $errors !== [] ? 'form--invalid' : '' ?>" action="add.php" method="post" enctype="multipart/form-data">
    ...
    <div class="form__container-two">
        <div class="form__item <?= getErrorClassname($errors, 'lot-name') ?>">
            ...
            <input id="lot-name" type="text" name="lot-name" placeholder="Введите наименование лота" value="<?= getPostVal($itemData, 'lot-name') ?>">
        ...
        <div class="form__item <?= getErrorClassname($errors, 'category_id') ?>">
            ...
            <select id="category" name="category_id">
                ...
                <?php foreach ($categories as $category): ?>
                <option value="<?= esc($category['category_id']) ?>" <?= $category['category_id'] === getPostVal($itemData, 'category_id') ? 'selected' : '' ?>><?= esc($category['category_name']) ?></option>
                <?php endforeach ?>
            </select>
    ...
    <div class="form__item form__item--wide <?= getErrorClassname($errors, 'description') ?>">
        ...
        <textarea id="message" name="description" placeholder="Напишите описание лота"><?= getPostVal($itemData, 'description') ?></textarea>
    ...
    <div class="form__container-three">
        <div class="form__item form__item--small <?= getErrorClassname($errors, 'lot-rate') ?>">
            ...
            <input id="lot-rate" type="text" name="lot-rate" placeholder="0" value="<?= getPostVal($itemData, 'lot-rate') ?>">
        ...
        <div class="form__item form__item--small <?= getErrorClassname($errors, 'lot-step') ?>">
            ...
            <input id="lot-step" type="text" name="lot-step" placeholder="0" value="<?= getPostVal($itemData, 'lot-step') ?>">
        ...
        <div class="form__item <?= getErrorClassname($errors, 'lot-date') ?>">
            ...
            <input class="form__input-date" id="lot-date" type="text" name="lot-date" placeholder="Введите дату в формате ГГГГ-ММ-ДД" value="<?= getPostVal($itemData, 'lot-date') ?>">
